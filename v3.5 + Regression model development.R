...
# Impute the missing values by node_name
...
##### Linear Regression modelling by Mete ######
# Package calls
library(stats)
...
# library(caTools)
# 
# split <- sample.split(mac_new$mac_label, SplitRatio = 0.7)
# 
# split
# 
# training_data <- subset(mac_new, split = "TRUE")
# testing_data <- subset(mac_new, split = "FALSE")
# 
# x <- as.numeric(mac_new$snrdn_mean)
# x <- as.numeric(x)
# class(x)
# 
# model_test <- lm(x~., data = training_data)
# vif(model_test)

# 1. Variable Definitions
# The estimator (independent) variables;
x1 <- as.numeric(mac_new$perc_snrdn_na)
x1 <- as.numeric(x1)
class(x1)

x2 <- as.numeric(mac_new$perc_pathloss_na)
x2 <- as.numeric(x2)
class(x2)

# Snrdn
x3 <- as.numeric(mac_new$snrdn_min)
x3 <- as.numeric(x3)
class(x3)

x4 <- as.numeric(mac_new$snrdn_max)
x4 <- as.numeric(x4)
class(x4)

x5 <- as.numeric(mac_new$snrdn_mean)
x5 <- as.numeric(x5)
class(x5)

x6 <- as.numeric(mac_new$snrdn_median)
x6 <- as.numeric(x6)
class(x6)

x7 <- as.numeric(mac_new$snrdn_sd)
x7 <- as.numeric(x7)
class(x7)

# Pathloss
x8 <- as.numeric(mac_new$pathloss_min)
x8 <- as.numeric(x8)
class(x3)

x9 <- as.numeric(mac_new$pathloss_max)
x9 <- as.numeric(x6)
class(x9)

x10 <- as.numeric(mac_new$snrdn_mean)
x10 <- as.numeric(x10)
class(x10)

x11 <- as.numeric(mac_new$snrdn_median)
x11 <- as.numeric(x11)
class(x11)

x12 <- as.numeric(mac_new$snrdn_sd)
x12 <- as.numeric(x12)
class(x12)

# Node Name;
x13 <- as.numeric(mac_new$node_name)
x13 <- as.numeric(x13)
class(x13)

# # Consecutive NAs
# x14 <- as.numeric(mac_new$pathloss_max_consecutive_NA)
# x14 <- as.numeric(x14)
# class(x14)

# The response (dependent / target) variable;
y <- mac_new$mac_label
y <- as.numeric(y)
class(y)

# 2. Apply the lm() function to represent the relation among the variables. Variable reduction: In this model, only the variables x1-x12 were taken into consideration.
# Linear model
model1 = lm(formula = y ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12, data = mac_new)

# For each src_node_id;
mac_new <-as.numeric(mac_new[[1]])

List_of_regression_models <- matrix(0, nrow(mac_new), 1)

for(i in 1:nrow(mac_new)){
  if(is.na(mac_new[i,"src_node_id"])){
    
    node <- mac_new[i,"node_name"]
    model_for_this_iteration = lm(formula = y ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12, data = mac_new)
    List_of_regression_models[i:1] <- model_for_this_iteration
  }
}

# Non-linear (Quadratic) model, where x_new is the 2-power value of pathloss_ccor.
x_new <- mac_new$pathloss_ccor^2
x_new <- as.numeric(x_new)
class(x_new)
model2 = lm(formula = y ~ x1 + x2 + x_new, data = mac_new)

# mac_label = A + B* V1 + C* V2 + ....

# 3. Report the model summary
anova(model1)
summary(model1)
plot(model1)
