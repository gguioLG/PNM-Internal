...
mac_snrdn_na <-
  aggregate(snr_dn ~ src_node_id, data = CPEData, function(x) {
    sum(is.na(x))
  }, na.action = NULL)
setnames(mac_snrdn_na, "snr_dn", "snr_dn_na")

mac_pathloss_na <-
  aggregate(pathloss ~ src_node_id, data = CPEData, function(x) {
    sum(is.na(x))
  }, na.action = NULL)
...
mac_pathloss <-
  CPEData[, c("node_name", "src_node_id", "hour_stamp", "pathloss")]
mac_pathloss <-
  subset(mac_pathloss,
         src_node_id %in% mac_feature_list_updated$src_node_id)
