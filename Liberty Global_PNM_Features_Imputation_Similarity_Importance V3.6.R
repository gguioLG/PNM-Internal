...
#Package Calls
library(RPostgreSQL, lib.loc = "/app/RPackageLibrary/R-3.3")
library(imputeTS, lib.loc = "/app/RPackageLibrary/R-3.3")
library(plyr, lib.loc = "/app/RPackageLibrary/R-3.3")
library(data.table, lib.loc = "/app/RPackageLibrary/R-3.3")
library(dplyr, lib.loc = "/app/RPackageLibrary/R-3.3")
library(stringr, lib.loc = "/app/RPackageLibrary/R-3.3")
library(reshape2, lib.loc = "/app/RPackageLibrary/R-3.3")
library(TSdist, lib.loc = "/app/RPackageLibrary/R-3.3")
library(caret, lib.loc = "/app/RPackageLibrary/R-3.3")
library(RColorBrewer, lib.loc = "/app/RPackageLibrary/R-3.3")
library(scales, lib.loc = "/app/RPackageLibrary/R-3.3")
library(ggplot2, lib.loc = "/app/RPackageLibrary/R-3.3")
library(corrplot, lib.loc = "/app/RPackageLibrary/R-3.3")
...
# rm(melted)
 ...
 # Creating a data frame with selected feature
 
mac_df <- mac_new[ , c("node_name", "src_node_id","perc_snrdn_na", "perc_pathloss_na",
                       "snrdn_min", "pathloss_min","snr_ratio", "pathloss_txpwrup_ratio",
                       "avg_time_3NA_snr", "avg_time_3NA_pathloss","avg_oper_time_3NA_snr", "avg_oper_time_3NA_pathloss",
                       "snrdn_dtw", "pathloss_dtw","snrdn_ecud", "pathloss_ecud",
                       "snrdn_erp", "pathloss_erp","snrdn_edr", "snrdn_lcss",
                       "snrdn_fourier", "pathloss_fourier","snrdn_wav", "pathloss_wav",
                       "mac_label")]
 
 
 # correlationMatrix <- cor(mac_df[,-c(1,2,25)])
 
 # corrplot(cor(mac_df[,-c(1,2,25)]))
 
 
 set.seed(7)

 prin_comp <- prcomp(mac_df[, !(colnames(mac_df) %in% c("src_node_id",
                          "node_name","mac_label"))], scale. = T, center = T )
 summary(prin_comp)
 std_dev <- prin_comp$sdev
 pr_var <- std_dev^2
 prop_varex <- pr_var/sum(pr_var)
 ...
 # Select the first 6 PCs (cumulative proportion of 91%) 
 
 df_pca <- as.data.frame(prin_comp$x[,c(1:6)])
 df_pca$mac_label <- as.factor(mac_df$mac_label)
