...
NodeData <-
  DBI::dbGetQuery(
    con,
    "SELECT DISTINCT a.node_name, a.hour_stamp, AVG(a.snr_up) FROM TEST_CASES.test_cases_polling_data a, TEST_CASES.TEST_CASES_REFERENCE_DATA b where a.topo_node_type = 'CPE' and a.node_name = b.node_name GROUP by a.node_name, a.hour_stamp ORDER by a.node_name, a.hour_stamp; "
  )
...
NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_kalman = na.kalman(avg))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_mean = na.mean(avg, option = "mean"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_median = na.mean(avg, option = "median"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_mode = na.mean(avg, option = "mode"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_random = na.random(avg))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_locf = na.locf(avg, option = "locf"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_nocb = na.locf(avg, option = "nocb"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_interp_linear = na.interpolation(avg, option = "linear"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_interp_spline = na.interpolation(avg, option = "spline"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_ma_expo = na.ma(avg, k = 4, weighting = "exponential"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_ma_linear = na.ma(avg, k = 4, weighting = "linear"))

NodeData_n <- NodeData_n %>%
  group_by(node_name) %>%
  mutate(avg_ma_sma = na.ma(avg, k = 4, weighting = "simple"))

# Calculate the Root Mean Square error after imputation

rmse_kalman <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_kalman))^2)
rmse_mean <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_mean))^2)
rmse_median <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_median))^2)
rmse_mode <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_mode))^2)
rmse_random <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_random))^2)
rmse_locf <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_locf))^2)
rmse_nocb <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_nocb))^2)
rmse_interp_linear <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_interp_linear))^2)
rmse_interp_spline <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_interp_spline))^2)
rmse_ma_expo <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_ma_expo))^2)
rmse_ma_linear <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_ma_linear))^2)
rmse_ma_sma <- sqrt((mean(NodeData_n$avg, na.rm = T) - mean(NodeData_n$avg_ma_sma))^2)
...
mac_snrdn_n <-
  CPEData[, c("node_name", "src_node_id", "hour_stamp", "snr_dn")]
mac_snrdn <-
  subset(mac_snrdn,
...
mac_pathloss_n <-   CPEData[, c("node_name", "src_node_id", "hour_stamp", "pathloss")]
